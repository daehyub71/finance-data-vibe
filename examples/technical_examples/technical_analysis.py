...
def main():
    ...
    print("🚀 Finance Data Vibe - 가치투자 기술적 분석 시스템")
    ...
    try:
        ...
        while True:
            ...
            if choice == '0':
                ...
                # 전체 매수 기회 스캔
                top_n = int(input("상위 몇 개 종목을 표시할까요? (기본값: 20): ").strip() or "20")
                opportunities = analyzer.scan_value_buying_opportunities(top_n)
                
            elif choice == '3':
                # 포트폴리오 리밸런싱
                symbols_input = input("포트폴리오 종목들을 입력하세요 (쉼표로 구분, 예: 005930,000660,035420): ").strip()
                if symbols_input:
                    symbols = [s.strip() for s in symbols_input.split(',')]
                    analyzer.create_portfolio_rebalancing_plan(symbols)
            
            elif choice == '4':
                # 기술적 차트 생성
                symbol = input("차트를 생성할 종목코드를 입력하세요 (예: 005930): ").strip()
                if symbol:
                    result = analyzer.analyze_stock_timing(symbol)
                    if result:
